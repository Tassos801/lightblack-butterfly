...
// Initialize function - handles all setup
function init() {
    ...
    // ============================================
    // Panel Case Studies Observer
    // ============================================
    const infoScrollContainer = document.getElementById('infoScrollContainer');
    const panelCases = document.querySelectorAll('.panel-case');

    const panelObserverOptions = {
        root: document.getElementById('infoPanel'),
        rootMargin: '-5% 0px -15% 0px',
        threshold: 0.1
    };

    const panelCaseObserver = new IntersectionObserver((entries) => {
    ...
    panelCases.forEach(caseEl => {
        panelCaseObserver.observe(caseEl);
    });

    const resetPanelCases = () => {
        panelCases.forEach(caseEl => {
            caseEl.classList.remove('visible');
        });
    ...
    // ============================================
    // Elegant Butterfly Fade In/Out
    // ============================================
    const fadeButterfly = (fadeIn) => {
        if (!butterfly) return;

        if (fadeIn) {
            butterfly.style.transition = 'opacity 0.8s ease, transform 0.8s ease';
            butterfly.style.opacity = '1';
        } else {
            butterfly.style.transition = 'opacity 0.6s ease, transform 0.6s ease';
            butterfly.style.opacity = '0';
        }
    };

    // Update info button to handle butterfly fade and case reset
    infoBtn.addEventListener('click', () => {
        const isOpening = !body.classList.contains('panel-open');

        if (isOpening) {
            fadeButterfly(false);
        } else {
            resetPanelCases();
            setTimeout(() => fadeButterfly(true), 300);
        }
...
// Handle DOM ready - works whether document is already loaded or still loading
